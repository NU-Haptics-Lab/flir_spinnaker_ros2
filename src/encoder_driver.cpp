...
#include <flir_spinnaker_ros2/encoder_driver.h>
...
: Node("encoderdriver", options)
{
  ...
  encoder->setParameters((rclcpp::Node*) this);
  encoder->setInputFormat(AV_PIX_FMT_BAYER_RGGB8);
  encoder->setPixFormat(AV_PIX_FMT_YUV420P);
  encoder->setUseHWFrames(true);
}
  ...
  while (keepRunning_ && rclcpp::ok()) {
    {
      ...
      if (!is_init_encoder){
        ...
        if(encoder->initialize(img->width_, img->height_, std::bind(&CameraDriver::packetReady, this, _1))){
            is_init_encoder = true;
        }
      }
      if (is_init_encoder && img && keepRunning_ && rclcpp::ok()) {
        doPublish(img);
      }
    }
  }
...
void CameraDriver::doPublish(const ImageConstPtr & im)
{
  ...
  // put into cv mat
  // need img.cols, img.rows, img.data, 
  cv::Mat img = cv::Mat((int) im->height_, (int) im->width_, CV_8U, const_cast<void *>(im->data_));
  encoder->encodeImage(img, imageMsg_.header, t0);
